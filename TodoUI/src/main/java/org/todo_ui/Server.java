...
public class Server implements Runnable {
    ...
    private void sendResponse(OutputStream outputStream, Path path) throws IOException {
        ...
        if (Files.exists(path)) {
            ...
            sendResponse(outputStream, bytes, 200);
        } else {
            sendResponse(outputStream, "404 Not Found!".getBytes(), 404);
        }
    }

    private void sendResponse(OutputStream outputStream, byte[] bytes, int port) throws IOException {

        outputStream.write(STR."HTTP/1.1 \{port} OK\r\n".getBytes());
        outputStream.write("\r\n".getBytes());
            ...
            while (true) {
                try (Socket accept = server.accept()) {
                    if (!Files.exists(rootPath)) {
                        sendResponse(accept.getOutputStream(), "404 Not Found!".getBytes(), 404);
                    }
